...
namespace tyrion
{
  Process::Process(std::string command, bool system, int timeout)
    : command_(command), system_(system), timed_out(false)
  {
    pipe(infd);
    pipe(outfd[0]);
    pipe(outfd[1]);
    ...
    // We add an extra second because of the read sleep
  ...
  void Process::Run()
  {
    ...
    if(pid_ == 0)
    {
      ...
      if (!issue)
      {
        ...
        if (sg != 0)
        {
          std::cerr << Error::Create("group.set", "Unable to set gid");
          issue = true;
        }
      }
      ...
      if (!issue)
      {
        ...
        if (su != 0)
        {
          std::cerr << Error::Create("user.set", "Unable to set uid");
          issue = true;
        }
      }
  ...
  bool Process::TimedOut()
  {
    ...
    time_t current_time;
    time(&current_time);
    timed_out = difftime(current_time, start_time) >= timeout_;
  ...
  int Process::Close()
  {
    ...
    for (int i = 0; i < 1000000 && rc <= 0; i+= 100000)
    {
  ...
  bool Process::SetUser(std::string name, bool set_group)
  {
  ...
  bool Process::SetGroup(std::string name)
  {
